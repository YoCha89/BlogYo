...
<div class="form">
	...
	<div class="blocForm">
		<form method="post" action="bootstrap.php?action=modifyAccount">
			...
			<input type="text" name="name" value="<?= nl2br(htmlspecialchars($account['name'])) ?>">
			</div>
			<?=isset($erreurs) && in_array(\Entity\Account::PRENOM_INVALIDE, $erreurs) ? 'Veuillez saisir un prénom.<br />' : ''?>
			<div class="champ">
			<label for="pseudo">Pseudo : </label>
			<input type="text" name="pseudo" value="<?= nl2br(htmlspecialchars($account['pseudo'])) ?>">
			</div>

			<?=isset($erreurs) && in_array(\Entity\Account::ALIAS_INVALIDE, $erreurs) ? 'Veuillez saisir un nom d\'utilisateur.<br />' : ''?>
			<div class="champ">
			<label for="email">Email : </label>
			<input type="text" name="email" value="<?= nl2br(htmlspecialchars($account['email'])) ?>">
			</div>
