...
{   
    ...
    // Displays the selected blogpost
    ...
    // Post a user comment on a blogpost
    ...
    public function executeModifyComment (HTTPRequest $request) {   
        ...
        if ($request->postExists('modKey')){
            ...
            'accountId' => $this->app->user()->getAttribute('id'),
            'author' => $this->app->user()->getAttribute('pseudo'),
            'blogPostId' => $request->postData('idB'),
            'content' => $request->postData('content'),
            'validated' => null
            ]);
    ...
    // Displays the list of comments written by a specific user
