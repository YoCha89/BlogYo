...
<form method="post" action="bootstrap.php?action=modifyComment">
	...
	<input id="idC" name="idC" type="hidden" value="<? echo nl2br(htmlspecialchars($comment['id'])) ?>">
	<input id="idB" name="idB" type="hidden" value="<? echo nl2br(htmlspecialchars($blogPost['id']))?>">
	
	<label for="content" class="form-label mt-4">Contenu :</label>
    <textarea class="form-control" id="content" name="content" rows="12"><?php echo nl2br(htmlspecialchars($comment['content'])) ?></textarea>
